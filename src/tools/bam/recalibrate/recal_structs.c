...
recal_init_info(const U_CYCLES cycles, recal_info_t *out_data)
{
	...
	new_vector_double(vector_size, 0.0, &(data->qual_empirical_Q));
	...
	new_vector_double(vector_size * cycles, 0.0, &(data->qual_cycle_empirical_Q));
	...
	new_vector_double(vector_size * NUM_DINUC, 0.0, &(data->qual_dinuc_empirical_Q));
	...
	for(i = 0; i < data->num_quals; i++)
	{
		for(j = 0; j < data->num_cycles; j++)
		{
			...
			if(data->qual_cycle_bases[matrix_index] != 0)
			{
				...
				data->qual_cycle_empirical_Q[matrix_index] = emp_Q;
		...
		for(j = 0; j < data->num_dinuc; j++)
		{
			...
			if(data->qual_dinuc_bases[matrix_index] != 0)
			{
				...
				recal_get_empirical_Q(data->qual_dinuc_miss[matrix_index], data->qual_dinuc_bases[matrix_index],
						data->qual_delta[i] + data->total_delta + estimated_Q, &emp_Q);
				delta = emp_Q - (data->qual_delta[i] + data->total_delta + estimated_Q);
				data->qual_dinuc_empirical_Q[matrix_index] = emp_Q;
...
recal_fprint_info(const recal_info_t *data, const char *path)
{
	...
	//Print general info
	fprintf(fp, "==============================\nGENERAL: \nTotal miss: %.2lf\nTotal bases: %lu\nTotal delta: %.2lf\nEstimated Q: %.2lf\nEmpirical Q: %.2lf\n",
			(double)data->total_miss, (U_BASES)data->total_bases, (double)data->total_delta, (double)data->total_estimated_Q, (double)data->total_empirical_Q);

	//Print quality infos
	fprintf(fp, "==============================\nQUAL VECTOR (Miss - Bases - Delta - Empirical):\n");
	for(i = 0; i < n_quals; i++)
	{
		fprintf(fp, "%3d %8.2f %10u %6.2f %6.2f \n",
				i + MIN_QUALITY, data->qual_miss[i] /*- SMOOTH_CONSTANT_MISS*/, data->qual_bases[i] /*- SMOOTH_CONSTANT_BASES*/,
				data->qual_delta[i] /*- SMOOTH_CONSTANT_MISS*/, data->qual_empirical_Q[i]);
	}
	...
	fprintf(fp, "==============================\nQUAL-CYCLE EMPIRICAL MATRIX (%d Quals - %d Cycles):\n", data->num_quals, data->num_cycles);
	for(i = 0; i < n_quals; i++)
	{
		fprintf(fp, "%d \t", i + MIN_QUALITY);
		for(j = 0; j < n_cycles; j++)
		{
			fprintf(fp, "%.2f \t", data->qual_cycle_empirical_Q[i * n_cycles + j]);
		}
		fprintf(fp, "\n");
	}
	...
	fprintf(fp, "==============================\nQUAL-DINUC EMPIRICAL MATRIX (%d Quals - %d Dinuc):\n", data->num_quals, data->num_dinuc);
	for(i = 0; i < n_quals; i++)
	{
		fprintf(fp, "%d \t", i + MIN_QUALITY);
		for(j = 0; j < n_dinuc; j++)
		{
			fprintf(fp, "%.2f \t", data->qual_dinuc_empirical_Q[i * n_dinuc + j]);
		}
		fprintf(fp, "\n");
	}
...
recal_save_recal_info(const recal_info_t *data, const char *path)
{
	...
	fwrite(&data->total_empirical_Q, sizeof(double), 1, fp);
	...
	fwrite(data->qual_empirical_Q, sizeof(double), data->num_quals, fp);
	...
	fwrite(data->qual_cycle_empirical_Q, sizeof(double), data->num_quals * data->num_cycles, fp);
	...
	fwrite(data->qual_dinuc_empirical_Q, sizeof(double), data->num_quals * data->num_dinuc, fp);
...
recal_load_recal_info(const char *path, recal_info_t *data)
{
	...
	fread(&data->total_empirical_Q, sizeof(double), 1, fp);
	...
	fread(data->qual_empirical_Q, sizeof(double), data->num_quals, fp);
	...
	fread(data->qual_cycle_empirical_Q, sizeof(double), data->num_quals * data->num_cycles, fp);
	...
	fread(data->qual_dinuc_empirical_Q, sizeof(double), data->num_quals * data->num_dinuc, fp);
